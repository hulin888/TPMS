...
class DATABASE:
    ...
    def insert_data(self,company,
        ...
        try:
            ...
            database_data['image'] = imagepath.split('TNMS_Live')[-1]
            database_data['vehicleno'] = str(vehicleno)
            database_data['time'] = time
            database_data['videoclip'] = videopath.split('TNMS_Live')[-1]
    ...
    def disable_reset_code(self, password_reset_token, username):
        try:    
            mongo_db_client = self.mongodb_url
            db = mongo_db_client[self.db]
            collection = db['codes']
            ...
            # Updating fan quantity form 10 to 25.
            filter = { "$and":[{'username': username},{'resetcode': password_reset_token}] }
            # Values to be updated.
            newvalues = { "$set": { 'status': 9 } }
    ...
    def insert_resetcodes(self, request_data, reset_code):
        try:
            mongo_db_client = self.mongodb_url
            db = mongo_db_client[self.db]
            collection = db['codes']

            request_data['resetcode'] = reset_code
            request_data['status'] = 1
            request_data['expired_in'] = datetime.utcnow()
            resp = collection.insert_one(request_data)
            resp = resp.inserted_id
            if resp:
                return resp
            else:
                return None
    ...
    def getvehiclefee(self, company, location, tollid, vehicletype, vehiclesubtype, cvrwithindtp):
        try:
            ...
            collection = db['toll_fee']
            myquery = {
                    "$and" :[{"company":company},{"location":location}, {'tollid': tollid}, {'vehicletype': vehicletype}, {'vehiclesubtype': vehiclesubtype}]
                    }
            if cvrwithindtp:
    ...
    def gettollfeedata(self, company, location, tollid):
        try:
            ...
            for doc in docs:
                ...
                doc['id'] = doc.pop('_id')
            df = pd.DataFrame(docs)
            for vehicletype, vdf in df.groupby('vehicletype'):
    ...
    def getdashboard_data(self, requestdata):
        ...
        cards_mapping = {"Car": "cars", "Truck": "trucks", "Bus": "trucks", "Lcv":"lcv"}
        ...
        try:
            ...
            db = mongo_db_client[self.db]
            collection = db['vehicle_detail']
            # company = requesdata['company']
            # location = requesdata['location']
            # tollid = requesdata['tollid']
            # laneno = requesdata['laneno']
            # vehicleno =requesdata['vehicleno']
            # vehicletype = requesdata['vehicletype']
            # vehiclesubtype = requesdata ['vehiclesubtype']
            # datefrom = requesdata ['datefrom']
            # dateto = requesdata ['dateto']
